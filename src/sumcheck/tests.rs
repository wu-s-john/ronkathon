...
use crate::{
  algebra::field::{prime::{PlutoBaseField, PlutoPrime}, Field},
  polynomial::multivariate_polynomial::{MultivariatePolynomial, MultivariateTerm, MultivariateVariable},
  random::Random, sumcheck::{verify_sumcheck_first_round, verify_sumcheck_last_round, verify_sumcheck_univariate_poly_sum},
};
...
fn test_full_sumcheck_protocol() {
    ...
    // The sumcheck protocol is used to prove the sum of a multivariate polynomial over a boolean hypercube
    // without explicitly computing all 2^n evaluations. This is particularly useful for large n,
    // where computing all evaluations would be computationally infeasible.

    // Step 1: Define the polynomial
    // We start with a multivariate polynomial because the sumcheck protocol is designed to work
    // with functions over boolean inputs, which are naturally represented as multivariate polynomials.
    let poly = MultivariatePolynomial::<PlutoBaseField>::from_terms(vec![
        ...
        MultivariateTerm::new(
            vec![MultivariateVariable { index: 1, exponent: 1 }, MultivariateVariable { index: 2, exponent: 1 }],
            -PlutoBaseField::ONE,
        ), // -x1 * x2
    ]);

    // Step 2: First round of the sumcheck protocol
    // The prover computes the actual sum over all boolean inputs and generates the first univariate polynomial.
    // This step is crucial because it reduces the n-variate polynomial to a univariate polynomial in x0,
    // while maintaining the property that its sum over {0,1} equals the original sum.
    ...
    let expected_poly1 = MultivariatePolynomial::<PlutoBaseField>::from_terms(vec![
        MultivariateTerm::new(vec![MultivariateVariable { index: 0, exponent: 1 }], PlutoBaseField::new(4)),
    ...
    // This polynomial represents the partial evaluation of the original polynomial with x0 fixed to the challenge value.
    ...
    let expected_poly2 = MultivariatePolynomial::<PlutoBaseField>::from_terms(vec![
        MultivariateTerm::new(vec![MultivariateVariable { index: 1, exponent: 1 }], PlutoBaseField::new(7)),
    ...
    // This check ensures that the evaluation of the first univariate polynomial at the challenge point
    // equals the sum of the second univariate polynomial over {0,1}.
    let (valid, _challenge) = verify_sumcheck_univariate_poly_sum(1, random_challenge1, &univariate_poly1, &univariate_poly2);
    assert!(valid, "Second round verification failed");

    // Step 4: Third (final) round of the sumcheck protocol
    // The process continues, further reducing the problem to a single point evaluation of the original polynomial.
    ...
    let expected_poly3 = MultivariatePolynomial::<PlutoBaseField>::from_terms(vec![
        MultivariateTerm::new(vec![], PlutoBaseField::new(16)),
    ]);
    assert_eq!(univariate_poly3, expected_poly3, "Final round polynomial is incorrect");

    // The verifier checks the final round
    // This check ensures that the evaluation of the second univariate polynomial at the challenge point
    // equals the sum of the third univariate polynomial over {0,1}.
    let (valid, _final_challenge) = verify_sumcheck_univariate_poly_sum(2, random_challenge2, &univariate_poly2, &univariate_poly3);
    ...
    // The verifier is convinced that the prover knows the correct sum without having to compute it directly.
